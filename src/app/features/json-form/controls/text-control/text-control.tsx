...
function TextControl(props: TextControlProps) {
  ...
  const { type } = schema;
  const { placeholder, readOnly } = uischema;

  const isNumberInput = type === "number";
  const hasErrors = errors.length > 0;
  const isDisabled = !enabled || readOnly === true;
