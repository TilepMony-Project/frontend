  ...
  try {
    ...
    for (const chainId of chainIds) {
      try {
        ...
        console.log(`[BridgeExecutor Worker] ✅ Chain ${chainId}: Found ${events.length} new events`);
      } catch (error) {
    ...
    if (pending.length > 0) {
      if (process.env.AUTO_EXECUTE_WORKFLOWS === 'true') {
        console.log(`[BridgeExecutor Worker] ⚡ Auto-executing ${pending.length} pending workflows...`);
        const result = await processAllPendingWorkflows();
        console.log(`[BridgeExecutor Worker] ✅ Executed: ${result.succeeded} succeeded, ${result.failed} failed`);
      } else {
    ...
    isRunning = false;
    console.log(`[BridgeExecutor Worker] 🏁 Polling cycle completed. Next poll in ${POLL_INTERVAL_MS/1000}s`);
  }
}
